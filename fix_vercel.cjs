...
// 2. Rewrite api/reset-quota.js using dynamic import
const resetQuotaCode = `
...
`;
fs.writeFileSync('api/reset-quota.js', resetQuotaCode.trim());

// 3. Rewrite scripts/daily-quota-reset.js
const dailyResetCode = `
async function run() {
    try {
        const { initializeApp } = await import("firebase/app");
        const { getFirestore, collection, getDocs, doc, updateDoc, getDoc, query, where, setDoc } = await import("firebase/firestore");
...
run();
`;
fs.writeFileSync('scripts/daily-quota-reset.js', dailyResetCode.trim());
